    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                (
                    "You are the Orchestration Planner for the AURA research "
                    "system. Your job is to break down the user's research "
                    "query into a series of clear, sequential, and atomic "
                    "steps. Always include steps for searching for evidence, "
                    "verifying claims, and generating a final report."
                ),
            ),
            ("human", "{query}"),
        ]
