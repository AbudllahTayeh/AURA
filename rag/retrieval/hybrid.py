...
def retrieve_hybrid(
    ...
    :param query: The search query string.
    :param corpus_chunks: A list of all text chunks (used for BM25).
    :param dense_results: A sorted list of dicts from Qdrant,
                          e.g., [{"id": 0, "score": 0.95}, ...]
    ...
    sparse_ranks = {
        chunk_idx: rank for rank, chunk_idx in enumerate(sparse_ranked_indices, start=1)
    }

    # 2. Dense Retrieval (Qdrant)
    # Assuming dense_results is already sorted descending by Qdrant's similarity score
    dense_ranks = {
        result["id"]: rank for rank, result in enumerate(dense_results, start=1)
    }
