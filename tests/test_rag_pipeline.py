...
def test_full_pipeline():
    ...
    sample_text = """
    Qdrant is an open-source vector database designed for fast and 
    ...
    Cross-encoders are highly accurate but computationally expensive, making them 
    ideal for the final reranking step in RAG systems.
    """
    ...
    # Lines 39-40
    # In a real scenario, you'd fetch dense_results from Qdrant via 
    # `client.search` or `client.query_points`.
    # For this test, we'll mock the dense results, assuming chunk 3 
    # (the cross-encoder sentence) scored highest.
    mock_dense_results = [
        {"id": 3, "score": 0.92},
        {"id": 2, "score": 0.65},
        {"id": 0, "score": 0.55},
    ...
    for res in hybrid_results:
        print(
            f"ID: {res['id']} | RRF Score: {res['rrf_score']:.4f} | "
            f"Text: {res['text'].strip()}"
        )
